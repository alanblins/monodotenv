...
func TestGetValue(t *testing.T) {
	...
	paths := []string{"mypath"}
	environments := map[string]string{"local": "value local"}
...
func TestGetValueUserFile(t *testing.T) {
	...
	secretsFile := models.SecretsYaml{}
	paths := []string{"mypath"}
	environments := map[string]string{"local": "USERLOCAL"}
	ev := models.EnvironmentVariable{
		Key:          "MYKEY",
		Source:       "user",
		Name:         "Any name",
...
func TestGetValueAESGCM(t *testing.T) {
	...
	secretsEnvironment := map[string]map[string][]string{"local": secretsEnvs}
	userfile := map[string]string{}
	secretsFile := models.SecretsYaml{
		Secrets: secretsEnvironment,
	}
	paths := []string{"mypath"}
	environments := map[string]string{"local": "7e57cc75b26e0e308c67eb81680572639868c51a3cbb"}
	ev := models.EnvironmentVariable{
		Key:          "MYKEY",
		Source:       "aes-gcm",
		Name:         "Any name",
